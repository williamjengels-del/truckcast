...
// Admin-scoped duplicate check for CSV imports.
//
// Mirrors /api/events/check-duplicates but parameterized on a target
// userId rather than the caller's auth.uid(). Admin-gated: non-admins
// cannot probe other users' event history through this endpoint.
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data: existingEvents, error } = await service
      .from("events")
    ...
    // Build lookup map: "event_name|event_date" -> existing event
